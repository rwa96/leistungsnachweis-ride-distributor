...
void RLAPSolverTest::uniqueEntries(const Tensor<int>& assignments, unsigned& uniqueRows, unsigned& uniqueCols){
...
int RLAPSolverTest::assignmentSum(const Tensor<int>& assignments, const Tensor<int>& inputMatrix){
    int result = 0;
...
TEST_P(RLAPSolverTest, TestingWithRLAPParams){
    ...
    const unsigned nAssignments = std::min(inputMatrix.getDims()[0], inputMatrix.getDims()[1]);
